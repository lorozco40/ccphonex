...
function traerdata(nuval = 0) {
    let cuenta = 0;
    let selected = '';
    pit.pag = (nuval != 0) ? (nuval-1)*pit.rpp : pit.pag;
    pit.bus = $("#buscar").val();
    $("#spinnerModal").modal("show");
    $.post(site_url+"pit/traerdata", { bus: pit.bus, pag: pit.pag, rpp: pit.rpp}, function(data) {
        ...
        if (!data.data || data.data.length == 0) {
                ...
                html += `<div class='table-row'>
                    ...
                    <div class='table-cell'>
                        <select id="select_user_pit_`+row.id+`" name="id_user" class="form-control" onchange="actualizar_usuario(`+row.id+`)">`;
                            ...
                            data.usuarios_pit.forEach((item,key) => {
            ...
            $("#tablaPit .table-row").remove();
            $("#tablaPit").append(html);
            paginacion(pit.pag, data.reg, pit.rpp, cuenta, 'paginacion', 'traerdata');
        }
...
function editar(id) {
    ...
    $.post(site_url+"pit/editar", { id: id}, function(data) {
        if (typeof data.error !== 'undefined') {
            ...
            $("#redi_vigencia_hora").val(data.redi_vigencia_hora);
