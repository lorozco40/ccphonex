...
function traerdatos() {
    $("#acum_diario").html("<tr><td class='text-center'><i class='fas fa-spinner fa-2x fa-spin'></i><span class='sr-only'>Cargando ...</span></td></tr>");
    var min = $("#min").val();
    var max = $("#max").val();
    $.post(site_url+"sl/acum_diario_data", $("#acumdiarioform").serialize(), function(data) {
        var html = "<tr><td style='color: #9a8a14;'>Sin datos con esos parámetros.</td></tr>";
        if (data.data.length == '' || data.data.length == null) {
            $("#acum_diario").html(html);
        } else {
            html = "<div class='table table-striped'><div class='table-header-group'>"+
                "<div class='table-cell'>Fecha</div><div class='table-cell'>ASA</div><div class='table-cell'>ABA</div>"+
                "<div class='table-cell'>Atendidas</div><div class='table-cell'>Por agentes</div><div class='table-cell'>AHT</div>"+
                "<div class='table-cell'>Prom ACW</div><div class='table-cell'>Abandonadas</div><div class='table-cell'>Salientes</div>"+
                "<div class='table-cell'>Prom salientes</div><div class='table-cell'>Prom entrantes</div><div class='table-cell'>% contestadas</div>"+
                "<div class='table-cell'>Aba < 3's</div><div class='table-cell'>SL("+data.data[0].seg+"'s)</div></div>";
