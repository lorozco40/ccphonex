...
foreach($tickets as $reporte) {
    ...
    $pdf->cell($cols[5],$row,'Ingeniero responsable:',$B,1,'L');
    $pdf->SetxY($X+$cols[13],$Y+$rows[1]);
    $pdf->cell($cols[7],$row,utf8_decode($reporte->ing_resp),1,1,'L');
    ...
    $pdf->cell($cols[4],$row,'Mantenimiento:',$B,1,'L');
    $pdf->SetxY($X+$cols[5],$Y+$rows[6]);
    ...
    $pdf->cell($cols[4],$row,'Fecha llegada:',$B,1,'L');
    $pdf->SetxY($X+$cols[5],$Y+$rows[16]);
    $pdf->cell($cols[4],$row,utf8_decode($reporte->llegada_f),1,1,'L');

    $pdf->SetxY($X+$cols[10],$Y+$rows[16]);
    $pdf->cell($cols[3],$row,'Hora llegada:',$B,1,'L');
    $pdf->SetxY($X+$cols[13],$Y+$rows[16]);
