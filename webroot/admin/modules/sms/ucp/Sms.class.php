...
class Sms extends Modules{
	...
	public function getOldMessages($id,$from,$to) {
	...
	public function getMessageHtmlByID($id) {
		...
		foreach($medias as $data) {
			switch($data['type']) {
				...
				break;
				case "text":
					$data['data'] = function_exists('mb_convert_encoding') ? mb_convert_encoding($data['data'],'UTF-8','UTF-8') : htmlentities($data['data']);
	...
	function ajaxHandler() {
		...
		switch($_REQUEST['command']) {
			case 'upload':
				foreach ($_FILES["files"]["error"] as $key => $error) {
					if ($error == UPLOAD_ERR_OK) {
						...
						$supported = array("png", "jpg", "jpeg", "gif", "tiff", "pdf", "vcf", "mp3", "wav", "ogg", "mov", "avi", "mp4", "m4a", "ical", "ics");
			...
			case 'send':
				...
				if ($_POST['message'] != '' && \FreePBX::Modules()->checkStatus("blacklist") && ($this->objSmsplus) && strtoupper(substr($_POST['message'], 0,5)) == 'BLOCK') {
					$addNumberToBlacklist = $this->objSmsplus->addNumberToBlacklist($_POST['message'], $_POST['to']);
					if ($addNumberToBlacklist) {
						if(!class_exists('FreePBX\modules\Sms\AdaptorBaseSms')) {
							include_once dirname(dirname(__FILE__)) . '/includes/AdaptorBaseSms.class.php';
						}
						$AdaptorBaseSms = new \FreePBX\modules\Sms\AdaptorBaseSms();
						$adaptorName = $this->sms->getAdaptorNameByDID($did);
						$result = $AdaptorBaseSms->sendMessage($_POST['to'],$this->formatNumber($did),$name,$_POST['message'], null, $adaptorName, 'sms-'.uniqid());
						if (!empty($result)) {
							$return['status'] = true;
							$return['id'] = $result;
